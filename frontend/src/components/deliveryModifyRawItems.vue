...
const props = defineProps(['item', 'disableCheckBox2', 'showAddToInventory']);

const emit = defineEmits(['checkboxChange', 'updateRawItem','refresh']);
...
const deductQuantityMissing = async (item) => {
  if (quantityMissing.value >= 0 && props.item.maximum_quantity >= quantityMissing.value) {
    ...
    try {
      const response = await api.patch(`/deliveryRoutes/raw/${item}`, {
...
const toggleIncompleteDeliver2 = async() => {
...
const completeMissingQuantity = async (id, missingQuantity) => {
  try {
    const result = await api.patch(`/deliveryRoutes/deliveryCompletedRaw/${id}`, {
...
<template>
  <div>
    <div class="flex gap-2 items-center justify-center z-0">
      ...
      <input v-if="item.missing_quantity === 0"
      ...
      <button   v-if="item.missing_quantity === 0" 
...
>
      ...
      <button :disabled="props.disableCheckBox2" @click=toggleIncompleteDeliver2(props.item.id) v-if="props.item.missing_quantity !== 0" class="btn btn-sm bg-blue-200 border-none text-blue-800">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
      ...
      <dialog ref="dialogRef"  class="modal z-50">
        <div class="modal-box w-full">
          <div class="w-full">
            ...
            <div class="modal-action">
              <button class="btn" @click="toggleIncompleteDeliver(props.item)">Close</button>
