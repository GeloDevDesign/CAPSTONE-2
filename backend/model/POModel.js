...
const getTotalMissingQuantityCount = async () => {
  try {
      ...
      const rawMaterialsCount = parseInt(rawMaterialsResult.rows[0].missing_quantity_count, 10);
      const productsCount = parseInt(productsResult.rows[0].missing_quantity_count, 10);


      return rawMaterialsCount + productsCount;
  } catch (error) {
      throw error;
  }
};
